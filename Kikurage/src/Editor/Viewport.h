...
namespace Kikurage {
	void Viewport(Renderer* renderer) {
		...
		ImGui::PushID("Albedo");
		ImGui::Image((void*)renderer->GetAlbedoTexture().GetHandle(), ImVec2(width, height), ImVec2(0, 1), ImVec2(1, 0));
		ImGui::PopID();
		ImGui::End();

		ImGui::Begin("Specular", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
		ImGui::PushID("Specular");
		ImGui::Image((void*)renderer->GetSpecularTexture().GetHandle(), ImVec2(width, height), ImVec2(0, 1), ImVec2(1, 0));
		ImGui::PopID();
		ImGui::End();
	}
}
